...
for ii = 1:P.NumberOfFrames
    ...
    for ss = 1:length(P.SNRRange)
        ...
        switch P.ReceiverType
            ...
            case 'Correlator',
                y=(reshape(y,length(P.Sequence),P.NumberOfSymbols)).';
                rxsymbols = y*conj(P.Sequence).';
                rxbits = reshape(rxsymbols < 0,1,P.NumberOfSymbols);
            case 'Rake',
                rxsymbols=zeros(P.NumberOfSymbols,1);
                for f=1:P.RakeFingers
                    ycrop=y(f:P.NumberOfSymbols*length(P.Sequence)+f-1);
                    yresh=(reshape(ycrop,length(P.Sequence),P.NumberOfSymbols)).';
                    
                    rxsymbols= rxsymbols+(yresh*conj(P.Sequence).')*conj(himp(f));
                end
                rxbits = reshape(rxsymbols(1:P.NumberOfSymbols) < 0,1,P.NumberOfSymbols);
